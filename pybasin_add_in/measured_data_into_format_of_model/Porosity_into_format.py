...
input: 'zk_poro.csv', 'poro_complete.xls'
output 'parallel_run_wells.csv', 'porosity.csv'

comp_well.csv and compwell_complete.xls into format for reading in pybasin
"""
...
df_wellnumber = pd.read_csv(outi+'/zk_poro.csv')

# read complete strata 
df_depth = pd.read_excel('wellnumber_into_porosity/poro_complete.xls')
...
out = 'wellnumber_into_porosity/porosity.csv'

if os.path.isfile(out) is True:
    os.remove(out)

txtfile = open(out,'a+')
txtfile.writelines("well,depth,porosity,range,n,source,wellnumber,EX,NY\n")

# fills file with lines from df_join
for index, row in df_join.iterrows():    
    if type(df_join.ix[index,'porosity']) is (float or int):     
            well = str(df_join.ix[index,'wellname'])
            depth = str(df_join.ix[index,'depth'])
            porosity = str(df_join.ix[index,'porosity'])
            ranger = str(df_join.ix[index,'range'])
            n = str(df_join.ix[index,'n'])
            ...
            new_line = well+','+depth+','+porosity+','+ranger+','+n+','+source+','+wellnumb+','+ex1+','+ny1
        
            txtfile.writelines(new_line + "\n")
txtfile.close()

# output parallel_well as wells, which are having equal properties are deleted (drop_duplicate)
well_output = 'wellnumber_into_porosity/parallel_run_wells.csv'
df_par = df_join
